...
-(instancetype)initWithType:(PushSocketMessageType)type
    ...
    if(self = [super init]){
        _type = type;//1.消息类型
        _isDup = NO;
        _qos = ack ? PushSocketMessageQosAck : PushSocketMessageQosNone;
        _isRetain = NO;
...
+(instancetype)headerWithType:(PushSocketMessageType)type withIsAck:(BOOL)ack{
    return [[PushFixedHeader alloc] initWithType:type withIsAck:ack];
}
...
#pragma mark -- 写入消息头
-(void)writeHeadeWithData:(NSMutableData **)data withHeader:(PushFixedHeader *)header{
    //第一个字节
