...
-(NSDictionary *)toSign{
    ...
    if(!self.account || !self.account.length){
        @throw [NSException exceptionWithName:PUSH_PARAMS_ACCOUNT reason:@"接入帐号不能为空" userInfo:nil];
    }
    if(!self.token || !self.token.length){
        @throw [NSException exceptionWithName:@"token" reason:@"接入密钥不能为空" userInfo:nil];
    }
    if(![self.account isEqualToString:(_params[PUSH_PARAMS_ACCOUNT])]){
        @throw [NSException exceptionWithName:PUSH_PARAMS_ACCOUNT reason:@"account未配置到参数集合中!" userInfo:nil];
    }
...
-(NSString *)toSignWithParams:(NSDictionary *)params withToken:(NSString *)token {
    ...
    [sortedKeys enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx, BOOL * _Nonnull stop) {
        if([key isEqualToString:PUSH_PARAMS_SIGN]) return;//0.关键字为sign的排除
